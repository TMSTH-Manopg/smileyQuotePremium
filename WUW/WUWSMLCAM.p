...
DEFINE VARIABLE nv_CampaignRefer  AS CHAR   FORMAT "X(50)".
...
DO i = 1 TO oArray:Length:
    ...
    CREATE ttSmileyRequestList.
        ...
        ttSmileyRequestList.CampaignRefer          =  STRING(oRoot:GetCharacter("publishCampaignRefer")) .
        ttSmileyRequestList.publishDate            =  STRING(oRoot:GetCharacter("publishCampaignDate")) .
        ttSmileyRequestList.EffectivePeriod        =  STRING(oRoot:GetCharacter("EffectivePeriod")) .
        ttSmileyRequestList.ExpiredPeriod          =  STRING(oRoot:GetCharacter("ExpiredPeriod")) .
        ttSmileyRequestList.CampaignName           =  STRING(oRoot:GetCharacter("CampaignName")) .
...
cDateTime    = STRING(YEAR(TODAY), "9999")
             ...
             + STRING(DAY(TODAY), "99") 
             + STRING(TRUNCATE(TIME / 3600, 0), "99")      /* ชั่วโมง */
             + STRING(TRUNCATE((TIME MOD 3600) / 60, 0), "99") /* นาที */
...
//   สร้าง Log Read File 
/*-
FIND FIRST   gw_safe.fcamp_fil WHERE gw_safe.fcamp_fil.CampCode  =  publishID  AND  
                                     gw_safe.fcamp_fil.Endtime   =  STRING(TIME,"HH:MM:SS")    NO-ERROR . 

IF NOT AVAIL gw_safe.fcamp_fil THEN  DO: -*/

      CREATE gw_safe.fcamp_fil.
      ASSIGN gw_safe.fcamp_fil.CampCode         =    publishID
             gw_safe.fcamp_fil.CampName         =    nv_CampaignCode
        //    gw_safe.fcamp_fil.EffectiveDate    =    ?
        //    gw_safe.fcamp_fil.ExpiryDate       =    ?
             gw_safe.fcamp_fil.cpStatus         =    ""
             gw_safe.fcamp_fil.btyp             =    nv_CampaignRefer
             gw_safe.fcamp_fil.Trndat           =    TODAY
             gw_safe.fcamp_fil.puserid          =    LDBNAME(1)
             gw_safe.fcamp_fil.entdat           =    TODAY
             gw_safe.fcamp_fil.Endtime          =    STRING(TIME,"HH:MM:SS")
