/****************************************************************************
 SmileyQuote.p   : -    Background process อ่านและ process Campaign file  SmileyQuote
                ---------------------------------------------------------
 Copyright    : Tokio Marine Safety Insurance (Thailand) Public Company Limited
                --------------------------------------------------------- 
 Method Name  :                                           
                --------------------------------------------------------- 
  CREATE BY    :   Manop G .  Background process อ่านและ process Campaign file  SmileyQuote
****************************************************************************/
...
DEF VARIABLE StatusProcess   AS CHARACTER .

DEFINE VAR fileout      AS CHAR . 
/*-  -*/
DEFINE VARIABLE publishID AS CHARACTER .        
{WUW\WUWSMLPRM.i}
DEFINE STREAM nfile.
DEFINE STREAM lfile.

fileout =   "D:\smileyquote\Log\03Process_SMLPRM_" + STRING(TODAY,"99999999") + STRING(MTIME,"99999999999") + ".log"  .

OUTPUT TO VALUE(fileout) APPEND.
PUT UNFORMATTED
    "[" STRING(TIME,"HH:MM:SS") "] Processing CampCode = START "SKIP.
OUTPUT CLOSE.
...
FOR EACH gw_safe.fcamp_fil
   WHERE gw_safe.fcamp_fil.Remark3 = "STATUS=NEW"
     ...
     /* In */
     // RUN logprocfile (INPUT gw_safe.fcamp_fil.Remark3 ) . 
    /* =========================
    ...
    publishID   =   gw_safe.fcamp_fil.CampCode .
...
OUTPUT TO VALUE(fileout) APPEND.
PUT UNFORMATTED
    "[" STRING(TIME,"HH:MM:SS") "] Processing CampCode = END "  SKIP.
OUTPUT CLOSE.  
...
FOR EACH tFile WHERE tFile.polmst <> "" NO-LOCK:
    ...
    OUTPUT TO VALUE(fileout) APPEND.
    PUT "[" STRING(TIME,"HH:MM:SS") "] Processing CampCode = "  
        tFile.polmst    SKIP.
    OUTPUT CLOSE.
...
IF AVAILABLE tFile AND tFile.makdes <> "" THEN DO:
...
nv_count2  =  nv_count2 + 1 .         
IF cFile <> "" THEN DO:
      IF INDEX(cFile,".") <> 0 THEN DO:
          nv_fileoutput = SUBSTR(cFile,1,INDEX(cFile,".") - 1) + "_PROC_Err" +
                     SUBSTR(cFile,INDEX(cFile,".")).
      END.
      ELSE DO:
          nv_fileoutput = cFile + "PROC_Err".
      END.
END.
...
nv_msg = "".
END PROCEDURE.      //  PROCEDURE PdOutputErrFile :      // #3

//-------------------------------------------
PROCEDURE logprocfile :
DEF INPUT PARAMETER StatusProcess   AS CHARACTER   .
/* =========================
    Log Process File 
   ========================= */
OUTPUT TO "D:\smileyquote\Log\02ConnectSML.txt" APPEND.
PUT TODAY    STRING(TIME,"HH:MM:SS") " : " StatusProcess    FORMAT "X(200)"  SKIP.
OUTPUT CLOSE.
END PROCEDURE. 
